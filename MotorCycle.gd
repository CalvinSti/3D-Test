...
func _physics_process(delta: float) -> void:
	...
	if not ability_active or not dead:
		...
		if ability_active:
			...
			if drifting:
				rotation.x = lerp(rotation.x, Input.get_axis("Left", "Right") * 1.1, delta * 1)
				#steering = move_toward(steering,Input.get_axis("Left", "Right") * steer, delta * 1)
				steering = -rotation.x/10
			#elif abs(rotation_degrees.z) >= 1:
				#angular_velocity = lerp(angular_velocity, -transform.basis.x*sign(rotation_degrees.x), 0.1)
			else:
				rotation.x = lerp(rotation.x, 0.0, delta * 2.5)
	...
	if Input.is_action_just_pressed("Ability") and not ability_active and not drifting:
		ability_active = true
	...
	if Input.is_action_pressed("drift"):
		...
		#for wheel in wheels:
			...
			#wheel.wheel_friction_slip = friction / 1.5
#
